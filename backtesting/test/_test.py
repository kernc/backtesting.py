...
class TestBacktest(TestCase):
    ...
    def test_broker_params(self):
        bt = Backtest(
            GOOG.iloc[:100], SmaCross, cash=1000, spread=0.01, margin=0.1, trade_on_close=True
        )
        bt.run()
    ...
    def test_spread_commission(self):
        ...
        self.assertEqual(
            stats["_equity_curve"]["Equity"].iloc[2:4].round(2).tolist(), [9685.31, 9749.33]
        )

        stats = Backtest(SHORT_DATA, S, cash=CASH, commission=(100, COMMISSION)).run()
        self.assertEqual(
            stats["_equity_curve"]["Equity"].iloc[2:4].round(2).tolist(), [9784.50, 9718.69]
        )
        ...
        self.assertEqual(
            stats["_equity_curve"]["Equity"].iloc[2:4].round(2).tolist(), [9781.28, 9846.04]
        )
    ...
    def test_commissions(self):
        ...
        FIXED_COMMISSION, COMMISSION = 10, 0.01
        ...
        EXPECTED_PAID_COMMISSION = (
            FIXED_COMMISSION
            + COMMISSION * SIZE * PRICE_ENTRY
            + FIXED_COMMISSION
            + COMMISSION * SIZE * PRICE_EXIT
        )
    ...
    def test_position_close_portion(self):
        ...
        bt = Backtest(GOOG, SmaCross, spread=0.002)
        bt.run()
    ...
    def test_multi_asset_run_and_symbols(self):
        n = 220
        goog = GOOG.iloc[:n].copy()
        half = GOOG.iloc[:n].copy()
        half[["Open", "High", "Low", "Close"]] = half[["Open", "High", "Low", "Close"]] * 0.5

        class S(_S):
            def next(self):
                i = len(self.data.index)
                if i == 20:
                    self.buy(symbol="GOOG", size=1)
                    self.buy(symbol="HALF", size=2)
                if i == 30:
                    self.position["GOOG"].close()
                if i == 40:
                    self.position["HALF"].close()

        stats = Backtest({"GOOG": goog, "HALF": half}, S, cash=1_000_000).run()
        self.assertEqual(len(stats._trades), 2)
        self.assertIn("Symbol", stats._trades.columns)
        self.assertEqual(stats._trades["Symbol"].tolist(), ["GOOG", "HALF"])

    def test_multi_asset_position_accessor(self):
        n = 120
        goog = GOOG.iloc[:n].copy()
        half = GOOG.iloc[:n].copy()
        half[["Open", "High", "Low", "Close"]] = half[["Open", "High", "Low", "Close"]] * 0.5
        ...
        class S(_S):
            def next(self):
                ...
                if i == 10:
                    self.buy(symbol="GOOG", size=1)
                    self.buy(symbol="HALF", size=1)
                if i == 11:
                    assert self.position["GOOG"].size == 1
                    assert self.position["HALF"].size == 1
    ...
    def test_multi_asset_misaligned_index_raises(self):
        n = 40
        goog = GOOG.iloc[:n].copy()
        shifted = GOOG.iloc[1 : n + 1].copy()
        with self.assertRaisesRegex(ValueError, "identical, aligned indexes"):
            Backtest({"GOOG": goog, "SHIFT": shifted}, SmaCross)
...
class TestOptimize(TestCase):
    ...
    def test_max_tries(self):
        ...
        for method, max_tries, random_state in (
            ...
            with self.subTest(method=method, max_tries=max_tries, random_state=random_state):
    ...
    def test_optimize_speed(self):
        ...
        print(end - start)
        handicap = 5 if "win" in sys.platform else 0.1
        self.assertLess(end - start, 0.3 + handicap)
...
class TestLib(TestCase):
    ...
    def test_compute_stats(self):
        ...
        self.assertNotEqual(list(stats._equity_curve.Equity), list(long_stats._equity_curve.Equity))
...
class TestDocs(TestCase):
    ...
    @unittest.skipUnless(
        sys.platform.startswith("linux"), "test_examples requires mp.start_method=fork"
    )
...
class TestRegressions(TestCase):
    ...
    def test_trade_on_close_closes_trades_on_close(self):
        ...
        df = pd.DataFrame({"Open": arr - 10, "Close": arr, "High": arr, "Low": arr})
        with self.assertWarnsRegex(UserWarning, "index is not datetime"):
    ...
    def test_trades_dates_match_prices(self):
        ...
        self.assertEqual(EURUSD.Close[trades["ExitTime"]].tolist(), trades["ExitPrice"].tolist())
