...
def plot(
    *,
    results: pd.Series,
    df: pd.DataFrame,
    assets: dict[str, pd.DataFrame] | None = None,
    ...
    trade_source = ColumnDataSource(
        dict(
            ...
            symbol=trades["Symbol"] if "Symbol" in trades else np.repeat("Asset", len(trades)),
        )
    )
...
return this.labels[index] || "";
    ...
    def new_indicator_figure(**kwargs):
        ...
        fig = new_bokeh_figure(
            x_range=fig_ohlc.x_range, active_scroll="xwheel_zoom", active_drag="xpan", **kwargs
        )
    ...
    def _plot_equity_section(is_return=False):
        ...
        if np.isnan(dd_end):
            ...
            if dd_end != equity.index[-1]:
                dd_end = np.interp(
                    equity[dd_start], (equity[dd_end - 1], equity[dd_end]), (dd_end - 1, dd_end)
                )
        ...
        fig = new_indicator_figure(
            y_axis_label=yaxis_label, **(dict(height=80) if plot_drawdown else dict(height=100))
        )
        ...
        fig.patch(
            ...
            source=ColumnDataSource(
                dict(
                    index=np.r_[index, index[::-1]], equity_dd=np.r_[equity, equity.cummax()[::-1]]
                )
            ),
            fill_color="#ffffea",
            line_color="#ffcb66",
    ...
    def _plot_pl_section():
        ...
        trade_source.add(
            np.take(["inverted_triangle", "triangle"], trades["Size"] > 0), "triangles"
        )
        r1 = fig.scatter(
            "index",
            "returns",
        ...
        set_tooltips(fig, tooltips + [("P/L", "@returns{+0.[000]%}")], vline=False, renderers=[r1])
    ...
    def _plot_superimposed_ohlc():
        ...
        resample_rule = (
            ...
            else dict(day="ME", hour="D", minute="h", second="min", millisecond="s").get(
                time_resolution
            )
        )
    ...
    def _plot_ohlc():
        """Main OHLC bars"""
        fig_ohlc.segment(
            "index", "High", "index", "Low", source=source, color="black", legend_label="OHLC"
        )
        r = fig_ohlc.vbar(
            "index",
            BAR_WIDTH,
    ...
    for f in figs:
        if f.legend:
            ...
            f.legend.background_fill_alpha = 0.9
        f.min_border_left = 0
        f.min_border_top = 3
...
def plot_heatmaps(
    ...
    dfs = [
        heatmap.groupby(list(dims)).agg(agg).to_frame(name="_Value") for dims in param_combinations
    ]
    figs: list[_figure] = []
    ...
    for df in dfs:
        ...
        fig = _figure(
            x_range=level1,  # type: ignore[call-arg]
            y_range=level2,
            x_axis_label=name1,
            y_axis_label=name2,
