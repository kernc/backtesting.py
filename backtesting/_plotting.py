...
def plot(
    *,
    ...
    if is_datetime_index:
        df, indicators, equity_data, trades = _maybe_resample_data(resample, df, indicators, equity_data, trades)
...
return this.labels[index] || "";
    ...
    ohlc_tooltips = [
        ...
        ("OHLC", NBSP.join(("@Open{0,0.0[0000]}", "@High{0,0.0[0000]}", "@Low{0,0.0[0000]}", "@Close{0,0.0[0000]}"))),
        ("Volume", "@Volume{0,0}"),
    ]
    ...
    def _plot_equity_section(is_return=False):
        ...
        if np.isnan(dd_end):
            ...
            if dd_end != equity.index[-1]:
                dd_end = np.interp(equity[dd_start], (equity[dd_end - 1], equity[dd_end]), (dd_end - 1, dd_end))
        ...
        fig = new_indicator_figure(y_axis_label=yaxis_label, **({} if plot_drawdown else dict(plot_height=110)))
        ...
        fig.patch(
            ...
            source=ColumnDataSource(
                dict(index=np.r_[index, index[::-1]], equity_dd=np.r_[equity, equity.cummax()[::-1]])
            ),
            fill_color="#ffffea",
            line_color="#ffcb66",
        )
    ...
    def _plot_drawdown_section():
        ...
        fig.scatter(
            argmax, drawdown[argmax], legend_label="Peak (-{:.1f}%)".format(100 * drawdown[argmax]), color="red", size=8
        )
    ...
    def _plot_pl_section():
        ...
        set_tooltips(fig, tooltips + [("P/L", "@returns_long{+0.[000]%}")], vline=False, renderers=[r1])
        set_tooltips(fig, tooltips + [("P/L", "@returns_short{+0.[000]%}")], vline=False, renderers=[r2])
    ...
    def _plot_superimposed_ohlc():
        ...
        resample_rule = (
            ...
            else dict(day="M", hour="D", minute="H", second="T", millisecond="S").get(time_resolution)
        )
    ...
    def _plot_ohlc():
        ...
        r = fig_ohlc.vbar("index", BAR_WIDTH, "Open", "Close", source=source, line_color="black", fill_color=inc_cmap)
        return r
    ...
    def _plot_indicators():
        ...
        def _too_many_dims(value):
            assert value.ndim >= 2
            if value.ndim > 2:
                warnings.warn(f"Can't plot indicators with >2D ('{value.name}')", stacklevel=5)
                return True
            return False
    ...
    fig = gridplot(
        plots, ncols=1, toolbar_location="right", toolbar_options=dict(logo=None), merge_tools=True, **kwargs
    )
...
def plot_heatmaps(
    ...
    if not (isinstance(heatmap, pd.Series) and isinstance(heatmap.index, pd.MultiIndex)):
        raise ValueError(
            "heatmap must be heatmap Series as returned by " "`Backtest.optimize(..., return_heatmap=True)`"
        )
    ...
    for df in dfs:
        ...
        fig = _figure(
            ...
            tooltips=[(name1, "@" + name1), (name2, "@" + name2), ("Value", "@_Value{0.[000]}")],
        )
    ...
    fig = gridplot(plots, ncols=ncols, toolbar_options=dict(logo=None), toolbar_location="above", merge_tools=True,)
