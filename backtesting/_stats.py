...
def compute_stats(
    ...
    equity: np.ndarray,
    ohlc_data: Union[pd.DataFrame, Dict[str, pd.DataFrame]],
    ...
    if isinstance(ohlc_data, dict):
        first_symbol = next(iter(ohlc_data))
        primary_ohlc_data = ohlc_data[first_symbol]
    else:
        first_symbol = None
        primary_ohlc_data = ohlc_data

    index = primary_ohlc_data.index
    dd = 1 - equity / np.maximum.accumulate(equity)
    dd_dur, dd_peaks = compute_drawdown_duration_peaks(pd.Series(dd, index=index))

    equity_df = pd.DataFrame(
        {"Equity": equity, "DrawdownPct": dd, "DrawdownDuration": dd_dur}, index=index
    )
    ...
    if isinstance(trades, pd.DataFrame):
        ...
        if isinstance(ohlc_data, dict):
            trades_df["Symbol"] = [getattr(t, "symbol", first_symbol) for t in trades]
    ...
    c = primary_ohlc_data.Close.values
    ...
    if is_datetime_index:
        ...
        have_weekends = index.dayofweek.to_series().between(5, 6).mean() > 2 / 7 * 0.6
        annual_trading_days = (
            52
            if freq_days == 7
            else 12
    ...
    if is_datetime_index:
        time_in_years = (
            s.loc["Duration"].days + s.loc["Duration"].seconds / 86400
        ) / annual_trading_days
        s.loc["CAGR [%]"] = (
            ((s.loc["Equity Final [$]"] / equity[0]) ** (1 / time_in_years) - 1) * 100
            if time_in_years
            else np.nan
        )  # noqa: E501
...
class _Stats(pd.Series):
    def __repr__(self):
        with pd.option_context(
            ...
            "display.max_rows",
            len(self),  # Reveal self whole
            "display.precision",
            5,  # Enough for my eyes at least
