...
#
# _Backtesting.py_ is a small and lightweight, blazing fast backtesting framework that uses state-of-the-art Python structures and procedures (Python 3.6+, Pandas, NumPy, Bokeh). It has a very small and simple API that is easy to remember and quickly shape towards meaningful results. The examples below focus on one tradeable asset at a time, which remains the simplest workflow for optimizing position entrance and exit signal strategies, decisions upon values of technical indicators, and interactive trade visualization. For shared-cash multi-asset simulations, see the `PortfolioBacktest` tutorial.
#
#
# ## Data
#
